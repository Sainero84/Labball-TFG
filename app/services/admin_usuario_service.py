...
def to_admin_cliente_response(user) -> AdminClienteResponseSchema:
    return AdminClienteResponseSchema(
...
def get_admin_clientes(db: Session) -> AdminClienteListResponseSchema:
    clientes = usuario_repository.get_all_clientes(db)

    return AdminClienteListResponseSchema(
        clientes=[to_admin_cliente_response(cliente) for cliente in clientes]
    )


def get_admin_cliente_by_id(
    db: Session,
    cliente_id: int
) -> AdminClienteResponseSchema:
    cliente = usuario_repository.get_cliente_by_id(db, cliente_id)

    if cliente is None:
        raise HTTPException(status_code=404, detail="Cliente no encontrado")

    return to_admin_cliente_response(cliente)
...
def create_admin_cliente(
    ...
    correo = cliente_data.correo.strip().lower()
    telefono = cliente_data.telefono.strip() if cliente_data.telefono else None
    ...
    try:
        ...
        cliente = usuario_repository.create_without_commit(
            db,
            {
                "firebase_uid": firebase_user.uid,
                "correo": correo,
        ...
        raise HTTPException(
            status_code=409,
            detail="No se pudo crear el cliente porque el correo o UID ya existe"
        ) from exception
    except HTTPException:
        db.rollback()
        ...
        raise HTTPException(
            status_code=502,
            detail="No se pudo crear la cuenta de cliente en Firebase Authentication"
        ) from exception

    return AdminClienteMessageResponseSchema(
        message="Cliente creado correctamente. Se ha enviado un correo para establecer la contrasena",
        cliente=to_admin_cliente_response(cliente)
    )
...
def update_admin_cliente(
    ...
    updated_fields = {}
    fields_set = cliente_data.model_fields_set

    if "correo" in fields_set:
        if cliente_data.correo is None:
        ...
        if existing_user is not None and existing_user.id_usuario != cliente_id:
            raise HTTPException(status_code=409, detail="Ya existe un usuario con ese correo")
        ...
        if correo != cliente.correo:
            try:
                ...
                if firebase_user.uid != cliente.firebase_uid:
                    raise HTTPException(
                        status_code=409,
                        detail="Ya existe una cuenta de Firebase Authentication con ese correo"
                    )
    ...
    if "telefono" in fields_set:
        updated_fields["telefono"] = (
            cliente_data.telefono.strip()
            if cliente_data.telefono else None
        )

    if not updated_fields:
        raise HTTPException(status_code=400, detail="No hay cambios para aplicar")

    correo_anterior = cliente.correo
    correo_actualizado = updated_fields.get("correo")
    ...
    try:
        ...
        if correo_actualizado is not None and correo_actualizado != correo_anterior:
            auth.update_user(cliente.firebase_uid, email=correo_actualizado)
    ...
    except auth.UserNotFoundError as exception:
        db.rollback()
        raise HTTPException(
            status_code=404,
            detail="No existe la cuenta de Firebase Authentication del cliente"
        ) from exception
    except IntegrityError as exception:
        db.rollback()
        raise HTTPException(
            status_code=409,
            detail="No se pudo actualizar el cliente porque el correo ya existe"
        ) from exception
    except HTTPException:
        db.rollback()
        raise
        ...
        raise HTTPException(
            status_code=502,
            detail="No se pudo actualizar la cuenta del cliente"
        ) from exception
...
def delete_admin_cliente(
    ...
    related_data = usuario_repository.get_related_data_counts(db, cliente_id)
    existing_relations = [
        f"{name}: {count}"
    ...
    if existing_relations:
        raise HTTPException(
            status_code=409,
            detail=(
                "No se puede eliminar el cliente porque tiene informacion de negocio asociada "
                f"({', '.join(existing_relations)}). "
                "Se recomienda conservar el historial."
            )
        )

    try:
        usuario_repository.delete_without_commit(db, cliente)
        db.flush()
        delete_firebase_user_if_exists(cliente.firebase_uid)
        db.commit()
    except HTTPException:
        db.rollback()
        raise
    ...
    return AdminClienteDeleteResponseSchema(message="Cliente eliminado correctamente")
